...
function Comment({username, comment, like_count, dislike_count}: Readonly<CommentInfo>){
	return (
...
export default function CommentModal({ postId, comment_count}: Readonly<CommentProps>) {
 ...
 function handleCommentModal() {
		(document.getElementById(`comment-popup-${postId}`) as HTMLDialogElement).showModal();
	}
			...
			<dialog id={`comment-popup-${postId}`} className="modal">
				<div className="comment-box modal-box h-[80%] max-w-[50%] bg-indigo-950 bg-opacity-90 p-0 flex flex-col justify-between">
					...
					<div className="comments px-8 flex flex-col gap-4">
						{comments.map((e: CommentInfo) => <Comment _id={e._id} username={e.username} comment={e.comment_string} 
						like_count={e.comment_like_list.length} dislike_count={e.comment_dislike_list.length}/>)}
					</div>
					<form className="sticky bottom-0 flex items-stretch w-full justify-between py-4 px-8 backdrop-blur-md gap-4"
